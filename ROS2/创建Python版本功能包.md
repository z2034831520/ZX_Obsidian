...
这里面报错表示找不到对应的文件，这就是程序在运行时查找[环境变量](../计算机基本概念/环境变量.md)导致的报错，因此接下来就需要执行`source install/setup.bash`命令来刷新一下当前终端的环境变量，把运行功能包所需的环境变量都加载到其中，主要是把`$PATH`加入到工作空间中。最后再运行相关指令即可查看到结果
![](assets/创建Python版本功能包/file-20260505204535446.png)