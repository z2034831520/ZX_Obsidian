...
要解释这个问题我们就需要简单的了解一下现代电脑的启动机制，现在的电脑主板普遍采用的是`UEFI`(Unified Extensible Firmware Interface，统一可扩展固件接口)引导模式，这种新的引导模式已经开始逐渐取代了老式的`BIOS`引导模式。如果你现在在电脑开机时连续按下键盘上的`F2`按键就会进入`UEFI`界面，你可能在网上听别人说过电脑重启后按下`F2/DEL`进入的是`BIOS`界面，为什么这里又说成是`UEFI`界面呢？在这种模式下，硬盘里面会有一个专门用来存放启动引导文件的独立空间，叫做`EFI`系统分区
## 实操
