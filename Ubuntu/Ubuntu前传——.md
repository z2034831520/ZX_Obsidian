...
虽然性能并不是很强劲，但是对于`Ubuntu`来说也够用了，我当时刚买回来的时候笔记本的硬盘容量是`512G`，一半用来装`Windows`另一半装了`Ubuntu`，后来感觉`Windows`的作用不是很大就全部刷成了`Ubuntu`，其中安装的版本是`22.04`，由于笔记本不是很新，所以这一次的安装没有像第一次那样出现掉驱动的问题，后面我就是采用一主一从的方案来解决这一