## 简介
在`CLion`的`STM32`嵌入式项目中我们在项目中添加了新的源文件或者新的文件夹之后，我们需要在项目中添加对应的文件信息，这步操作就是在给新建的文件和文件夹**上户口**，否则在编译的时候编译器就无法找到对应的文件，这也必然会导致报错，接下来我会介绍一下如何在`CLion`中添加对应的头文件搜索路径
