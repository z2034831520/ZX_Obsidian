...
## MCP服务
模型上下文协议（`Model Context Protocol`,简称`MCP`），这是一个开源的协议，作用是为`AI`应用提供接口，通过接口`AI`可以去获取外部数据。因为在早期`AI`几乎只能用来进行简单的问答，几乎无法去获取外部信息。早期我购买过`deepseek`的官方`API`，我把`API`接入了很多第三方软件中，例如`Cline`、`Cherry Studio`和
## 实操
