...
## MCP服务
模型上下文协议（`Model Context Protocol`,简称`MCP`），这是一个开源的协议，作用是为`AI`应用提供接口，通过接口`AI`可以去获取外部数据。因为在早期`AI`只能用来进行简单的问答，我购买过`deepseek`的官方`API`，我把`API`接入了很多第三方软件中，例如`Cline`、`Cherry Studio`以及`TavernAI`。在使用的过程中我发现我可以问一些复杂的编程知识或者新闻信息等等，但是他们却无法对文件进行直接修改，给人的感觉就像是`AI`手脚被困住了一样。后来我了解到了`MCP`协议，该协议是由`Anthropic`公司在`2024年11月25日`正式提出并开源的，只可惜当时的我只是简单的了解了一下，并没有进行深入的学习。本次我们将`obsidian`笔记提供给`codex`所使用的就是`MCP`协议
## 实操
