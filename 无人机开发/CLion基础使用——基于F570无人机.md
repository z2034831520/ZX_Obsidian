...
这里需要辨析一个问题：`FreeRTOS`中的定时器和系统的硬件定时器的区别，二者虽然都是定时器，但是一个是基于直接的硬件来实现的，另一个则是软件实现的。`FreeRTOS`虽然征用了系统的滴答定时器，但是任务调度涉及到的定时器却是纯软件实现的，这种纯软件实现的定时器功耗较高，且精度较低。因此如果在`FreeROTS`中出现了一个优先级非常高的任务，结合`FreeRTOS`的抢占式任务调度，软件定时器的回调函数就会被延后执行。而系统的硬件定时器虽然数量少，但是精度高，主要用于中断上下文控制，中断任务之间的抢占是相对平等的，即使是最低优先级的任务也能打断最高优先级的任务
