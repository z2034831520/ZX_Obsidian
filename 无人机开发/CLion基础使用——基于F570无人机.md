...
### 定时器参数读取
我们刚才初始化了对应的高精度硬件定时器之后接下来就要想办法去获取对应的数值了，由于我们后续需要根据时间戳来判断一个任务占据了多少CPU资源，因此我们需要去实时的读取定时器的计数值。读取计数值的操作系统会通过调用`getRunTimeCounterValue`函数来实现，该函数和定时器初始化函数一样，也是一个弱定义函数，我们同样可以在其它文件中去