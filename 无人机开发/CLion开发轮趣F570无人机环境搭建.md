...
因此我们关注的重点也就是文档中的`CMakeList.txt`，我们可以在该文件中看到程序的属性信息，如果学习过`CMake`相关知识的话那么对文件中的内容肯定不会陌生。后续我们需要在项目中创建文件夹并天降文件夹时就需要去修改该文件中的内容，直接修改`CMakeList.txt`文件对初学者并不友好，但是好处是我们需要用到的配置语句并不复杂，数量也不算多，因此我们可以先简单学习一下相关的`CMake`知识
