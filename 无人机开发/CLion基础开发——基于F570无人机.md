...
![](assets/CLion基础开发——基于F570无人机/file-20260729153005331%201.png)
...
![](assets/CLion基础开发——基于F570无人机/file-20260729153005333.png)
我们将对应的函数状态都切换成`Enable`即可。生成代码之后我们就可以在`CLion`中的`freertos.c`文件中看到对应的操作函数
![](assets/CLion基础开发——基于F570无人机/file-20260729153005332.png)
...
![](assets/CLion基础开发——基于F570无人机/file-20260729153005334.png)
...
2. 进入系统设置界面之后我们可以选择编辑器选项，然后在新界面中点击蓝色字体中的**实时模板**
	![](assets/CLion基础开发——基于F570无人机/file-20260729153005337.png)
3. 在随后的界面中选择语言类型为`C/C++`
	![](assets/CLion基础开发——基于F570无人机/file-20260729153005336.png)
4. 我们接下来点击新界面中的`+`号，随后我们就可以创建新的代码片段
	![](assets/CLion基础开发——基于F570无人机/file-20260729153005340.png)
5. 随后我们就可以编辑代码片段中的内容，左边是代码体，右边是片段的相关参数
	![](assets/CLion基础开发——基于F570无人机/file-20260729153005339.png)
...
![](assets/CLion基础开发——基于F570无人机/file-20260729153005341.png)
由于`STM32F405RGTx`系列`MCU`的`TIM6`挂载在低俗外设总线（`APB1`）上，结合当前程序的时钟树我们不难发现低速时钟总线上的主频为`84Mhz`
![](assets/CLion基础开发——基于F570无人机/file-20260729153005342.png)
...
```
大致的原理如下图所示：
![](assets/CLion基础开发——基于F570无人机/file-20260729153005343.png)
...
### 程序烧录
我们本次使用的烧录器是`ST-Link`，使用该烧录器在`CLion`中的开发体验要好于其它的烧录器，我们可以对烧录器进行快速配置，我们可以点击顶部的原生烧录选项
...
### 结果验证
烧录完成之后我们还要验证现在`MCU`是否能够实时输出对应的系统信息，此时就需要用串口来进行查看了。这里就不得不提到`CLion`的另一个强大功能了，它内置了串口插件，我们下载之后就可以直接在软件中查看串口信息了，这样就不用在`IDE`和串口监测软件之间反复横跳了，我们首先点击右上角的设置图标，然后点击插件选项
...
## 配置推荐
不知道大家平时的开发习惯是什么，就我个人来说我接触的一门开发语言是C语言，我们的开发习惯就是将大括号单开一行，但是`CLion`的默认配置是大括号不单开一行尤其是函数定义，默认的布局有点类似于`Java`的布局，这样的布局看起来会比较变扭，如果你和我的代码风格比较相似，或者你也不习惯官方的代码风格，那么我们就可以自己定义对应的代码格式，
