...
由于`STM32F405RGTx`系列`MCU`的`TIM6`挂载在低俗外设总线（`APB1`）上，结合当前程序的时钟树我们不难发现低速时钟总线上的主频为`84Mhz`，对应的参数信息可以在[无人机主控MCU定时器信息](无人机主控MCU定时器信息.md)文章中看到
...
### 结果验证
烧录完成之后我们还要验证现在`MCU`是否能够实时输出对应的系统信息，此时就需要用串口来进行查看了，对应的串口连接操作可以到[CLion内置串口界面使用](../CLion/CLion内置串口界面使用.md)文章中查看
串口连接之后我们即可看到对应的输出参数信息
...
## 配置推荐
不知道大家平时的开发习惯是什么，就我个人来说我接触的一门开发语言是C语言，我们的开发习惯就是将大括号单开一行，但是`CLion`的默认配置是大括号不单开一行尤其是函数定义，默认的布局有点类似于`Java`的布局，这样的布局看起来会比较变扭，如果你和我的代码风格比较相似，或者你也不习惯官方的代码风格，那么我们就可以自己定义对应的代码格式，对应的操作步骤可以参照[CLion大括号格式配置](../CLion/CLion大括号格式配置.md)文章中的内容
## 总结
