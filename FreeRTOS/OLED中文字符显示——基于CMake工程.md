...
```
该函数同属于`OLED`操作函数，因此我将它放在`driver_oled.c`文件中实现，为了能够输出一个完整的中文字符我们选择将其分为上下两部分来进行输出，每次先输出汉字的上半部分，然后再输出下半部分

### 函数调用
输出函数编写完毕之后我们就可以直接进行调用，我们可以在`main.c`中进行调用，调用结束之后我们可以