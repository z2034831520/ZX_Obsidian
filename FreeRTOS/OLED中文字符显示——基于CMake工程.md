...
![image.png](https://cdn.jsdelivr.net/gh/z2034831520/obsidian-image-bed@main/images/20260811174638658.png)
...
```
烧录程序之后可以看到对应的显示数据
![image.png](https://cdn.jsdelivr.net/gh/z2034831520/obsidian-image-bed@main/images/20260811174702447.png)
...
![image.png](https://cdn.jsdelivr.net/gh/z2034831520/obsidian-image-bed@main/images/20260811174726147.png)

我们可以点击顶部的选项，然后就会弹出对应的配置窗口了
该软件的配置如下：
![image.png](https://cdn.jsdelivr.net/gh/z2034831520/obsidian-image-bed@main/images/20260811174807624.png)
...
![image.png](https://cdn.jsdelivr.net/gh/z2034831520/obsidian-image-bed@main/images/20260811174818756.png)

函数中的X代表图中的X轴坐标，函数中的y参数代表图中的Y轴坐标

这里需要简单介绍一下为什么函数要这样写，前面的入口参数检查较为常规，因为刚才说过当前的`OLED`显示模块横向最多显示8个字符，纵向最多显示4个字符，因此我们在函数开始时检查一下坐标是否超出了`OLED`模块的表示范围，`SSD1306`在寻址模式下，把垂直方向上每8个像素组织成一个`Page`，其中
```txt
Page 0：第 0～7 行
Page 1：第 8～15 行
Page 2：第16～23 行
...
- 高16像素，因此占用两个`Page`
- 总像素为`16*16 = 256`
- 每个字节表示8个像素，因此需要`256 % 8=32`字节
因此我们选择将一个字符拆分成上下两个部分，
```txt
font[0]  ～ font[15]  ：上半部分8行，共16列
font[16] ～ font[31]  ：下半部分8行，共16列
```
拆分之后的结构图为：
```txt
16×16汉字
            ...
            0  1  2  ... 15
          ┌────────────────┐
上8行     │ font[0..15]    │  Page N
          ├────────────────┤
下8行     │ font[16..31]   │  Page N+1
          └────────────────┘
```

因此整个函数的本质可以看作是：
把汉字坐标换算成`OLED`地址，再把32字节字模按照上16字节、下16字节分别写入两个相邻`Page`
### 函数调用
输出函数编写完毕之后我们就可以直接进行调用，我们可以在`main.c`中进行调用，调用该函数的相关代码如下：
`OLED_PutChinese16(1, 0, font_zhong);`
我们在代码中调整了中文字符的显示位置，
程序修改并烧录后的输出效果如图所示：
![image.png](https://cdn.jsdelivr.net/gh/z2034831520/obsidian-image-bed@main/images/20260811174834773.png)
