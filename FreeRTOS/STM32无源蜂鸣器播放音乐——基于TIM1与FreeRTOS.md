...
最近在使用 `STM32F103C8T6` 进行开发时，我需要通过 `PA8` 引脚控制一个低电平触发的无源蜂鸣器。最开始只是让蜂鸣器发出声音，后面又继续尝试调整响度、播放不同音调，并把“晴天”工程中的音乐播放功能迁移到了当前项目。

无源蜂鸣器本身没有振荡源，需要单片机持续输出方波才能发声。改变方波频率可以改变音调，控制每个频率持续的时间则可以形成节奏。当前项目使用 `TIM1_CH1` 输出 PWM，再通过 FreeRTOS 的 `osDelay()` 控制音符时长。
...
TIM1 当前的参数配置为：
`TIM1`的主频为`72 MHz`，我们将预分频值设置为 `72-1` ，对应的定时器计数频率为：`72 MHz ÷ (71 + 1) = 1 MHz`

定时器每计数一次需要 `1 μs`。后续播放不同音调时不再修改预分频器的值，而是根据音符频率动态修改自动重装载器的值：
~~~text
ARR = 1,000,000 ÷ 音符频率 - 1
~~~

例如输出 `4000 Hz` 方波时，ARR 等于 249，这也就是 TIM1 初始化时将 `Period` 设置为 `250 - 1` 的原因。
